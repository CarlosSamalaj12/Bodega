...
// ========== EXPORTS ==========
// resolveStockScope - moved from server-auth.js (used by many route modules)
export async function resolveStockScope(user) {
  const userId = Number(user?.id_user || 0);
  const id_role = Number(user?.id_role || 0);
  const id_bodega = Number(user?.id_warehouse || 0);
  if (!id_bodega) {
    return {
      id_usuario: userId, id_bodega: null, maneja_stock: false, is_principal: false,
      is_bodeguero: false, is_report_role: false, is_admin_role: false,
      can_view_existencias: false, can_all_bodegas: false,
      has_warehouse_restrictions: false, allowed_warehouse_ids: [],
    };
  }
  const [[roleRow]] = await pool.query(
    "SELECT nombre_rol FROM roles WHERE id_rol=:id_rol LIMIT 1", { id_rol: id_role }
  );
  ...
  const [[bodRow]] = await pool.query(
    "SELECT b.tipo_bodega, b.nombre_bodega, COALESCE(cb.maneja_stock, 0) AS maneja_stock FROM bodegas b LEFT JOIN configuracion_bodega cb ON cb.id_bodega=b.id_bodega WHERE b.id_bodega=:id_bodega LIMIT 1",
    { id_bodega }
  );
  ...
  return {
    id_usuario: userId, id_bodega, maneja_stock, is_principal, is_bodeguero,
    is_report_role, is_admin_role, can_view_existencias,
    can_all_bodegas: is_report_role || is_admin_role,
    has_warehouse_restrictions, allowed_warehouse_ids,
  };
}
...
// Named exports for all shared dependencies
export {
  app, httpServer, HOST, PORT, io, pool, bcrypt, jwt, crypto, __dirname, __filename,
  opsMetrics, PERM_CATALOG, recentRequestSignatures,
  auth, signToken, requirePermission, emitPedidoChanged,
  requireSensitiveApproval, enforceDailyCloseBeforeMutations, readDashboardResumenCache,
  beginIdempotentRequest, buildRequestSignature, trackPinFailure,
  buildTokenizedLikeFilter, buildNamedInClause, normalizeWarehouseIdList,
  getScopedWarehouseFilter, buildProductWarehouseVisibilityClause,
  getUserWarehouseAccessIds, getProductVisibleWarehouseIds, saveProductVisibleWarehouseIds,
  isProductVisibleInWarehouse, getActiveWarehouseIds, setProductWarehouseVisibility,
  areWarehouseIdsValid, getUserPermissionsMap, canManageUserPermissions, userHasPermission,
  ...
  buildWarehouseFooterHtml, getPrintLogoDataUri,
  verifySensitiveApproval, toSensitiveApprovalPayload, writeSensitiveActionAudit,
  createDailyCloseForDate, normalizeLogoData, isAvatarTableMissingError,
  isWarehouseLogoTableMissingError,
  ymd, dmy, normalizeYmdInput, addDaysYmd, onlyToday,
  normalizeDeviceKey, getSharedDeviceKeys, isValidOrderPin, isValidSupervisorPin,
  findOrderPinCollision, clampText, numMoney, numQty,
  ...
  OPS_ALERT_WINDOW_MS, OPS_PIN_WINDOW_MS, OPS_BACKUP_AUTO_ENABLED,
  OPS_BACKUP_INTERVAL_MS, OPS_BACKUP_BASE_DIR, OPS_RECOVERY_CHECK_INTERVAL_MS,
  IDEMPOTENCY_WINDOW_MS,
  listActive, softDelete, createLogicalBackup, maybeRunMonthlyRecoveryTest, prewarmDashboardCache,
  ...
  ensureWarehouseLogoTable, ensureBodegaContactColumns,
  ...
  ensureMovimientoPastUpdateTrigger, ensureCuadreCajaTable,
  ensureDashboardCacheTable, ensureUserAvatarTable,
  ensureUserOrderPinTable, ensureSupervisorPinTable,
  ensureUsersNoAutoLogoutColumn, ensureDailyCloseTables,
  ensureOpsAuditTables, ensureSensitiveActionAuditTable,
  ensureOrderDispatchColumns,
};
